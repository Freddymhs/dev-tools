...
const submenu = async (category) => {
  const choices =
    category === "code"
      ? [
          { title: "Generar resume", value: "generate" },
          { title: "Generar + dividir", value: "generate-split" },
          { title: "Solo dividir markdown", value: "split" },
...
const askVar = async (key, label, currentValue) => {
  const { value } = await prompts(
    {
      type: "text",
      name: "value",
