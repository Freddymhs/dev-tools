...
const generateResume = (ctx, projectPath) => {
  const env = makeEnv(ctx.dir, { PROJECT_PATH: projectPath });
  return runScript('generate_resume.cjs', env);
};
...
describe('split_markdown.cjs', () => {
  ...
  test('divide RESUME.md en partes en CODE_PARTS', () => {
    // Primero genera el RESUME
    generateResume(ctx, path.join(FIXTURES, 'project'));
  ...
  test('segunda ejecución limpia partes viejas (Bug stale parts)', () => {
    ...
    fs.writeFileSync(path.join(partsDir, 'RESUME_part03.md'), 'stale', 'utf8');
    fs.writeFileSync(path.join(partsDir, 'RESUME_part04.md'), 'stale', 'utf8');
  ...
  test('falla con mensaje claro si RESUME.md no existe', () => {
